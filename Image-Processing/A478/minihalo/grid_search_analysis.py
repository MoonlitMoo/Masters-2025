...
OUTPUT_DIR = "grid_flux_images"
GRID_DIR = "grid_search"
os.makedirs(OUTPUT_DIR, exist_ok=True)

images = sorted([f.split(".image.tt0")[0] for f in os.listdir(GRID_DIR) if ".image.tt0" in f])
masks = []

# Get all the different masks
for im in images:
    image_file = f"{GRID_DIR}/{im}.image.tt0"
    residual_file = f"{GRID_DIR}/{im}.residual.tt0"
    ...
    # Get minihalo flux
    # Need to seed the centre since the head-tail is large.
...
for im in images:
    ...
    f, e, s = get_minihalo_flux_density(f"{GRID_DIR}/{im}", out_img, mask=base_mask, agn_err=8.87e-6)
    flux.append(f)
    error.append(e)
    sigma.append(s)

# Plotting via ChatGPT
pat = re.compile(r"robust([\-0-9.]+)_ssb([\-0-9.]+)")
rows = []
for fname, flux_jy, error_jy, rms in zip(images, flux, error, sigma):
    m = pat.search(fname)
    if not m:
        raise ValueError(f"Could not parse robust/ssb from: {fname}")
