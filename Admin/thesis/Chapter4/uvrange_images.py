...
BASE_DIR = "/media/benja/T7/tmp3/"
OUT_DIR = "FITS"
images = [f"image_{i}" for i in [5, 10, 15]]

fits_paths = ["../Chapter5/vla_images/FITS/rxj1720.fits"]
for im in images:
...
norm = None
for i, (fs, ax, label) in enumerate(zip(fits_paths, axs, [0, 5, 10, 15])):
    do_x = i > 1
    do_y = i == 0 or i == 2
    _, _, norm = plot_fits_on_ax(
        fs, ax, norm=norm, rms=20e-6/3, zoom=20, alpha=0.01,
        show_xlabel=do_x,
        show_ylabel=do_y,
    )
...
plt.savefig("uvrange_image.pdf", dpi=300, bbox_inches='tight')