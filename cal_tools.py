...
from casatasks import flagmanager, gencal, plotweather, clearcal, applycal
...
def clean_flag_versions(dry_run=False):
    """ Removes all flag versions not part of the flagging steps. Use with caution. """
    versions = flag_versions()
    extra_versions = set(versions) - set(FLAGGING_STEPS)
    if dry_run:
...
def remove_calibration(target: str, dry_run=False):
    """
...
def primary_pre_calibration(flag_version=FLAGGING_STEPS[1]):
    ...
    if flag_version in flag_versions():
    ...
    remove_calibration('G0')
    gaincal(vis=msname, caltable=f'{name}.G0', field=p_calibrator, refant=refant, spw=spw_string,
            gaintype='G', calmode='p', solint='int', minsnr=5, gaintable=apriori_tables())
    # Get the delay calibration
    remove_calibration('K0')
    gaintables = apriori_tables() + [f'{name}.G0']
    ...
    remove_calibration('G1')
    gaintables = apriori_tables() + [f'{name}.B0', f'{name}.K0']
...
def primary_post_calibration(flag_version=FLAGGING_STEPS[2]):
...
def secondary_pre_calibration(flag_version=FLAGGING_STEPS[3], primary_flag_version=FLAGGING_STEPS[2]):
    primary_post_calibration(primary_flag_version)
    if flag_version in flag_versions():
        flagmanager(vis=msname, mode='restore', versionname=flag_version)
    else:
        raise ValueError(f"Flagging version '{flag_version}' not found in flagmanager.")
    ...
    gaincal(vis=msname, caltable=f'{name}.G2', field=s_calibrator,
            ...
            gaintable=gaintables,
            gainfield=secondary_gainfields(gaintables),
            interp=interp_modes(gaintables),
            append=True)
    # Add the fluxmodel to secondary calibrator
    remove_calibration("fluxscale1")
...
def secondary_post_calibration(flag_version=FLAGGING_STEPS[4], primary_flag_version=FLAGGING_STEPS[2]):
    """ Re runs the secondary calibration using the post flagged secondary version. """
    secondary_pre_calibration(flag_version, primary_flag_version)

def apply_initial_calibration():
    """ Applies initial calibration to the primary calibrator. """
    gaintables = apriori_tables() + [f'{name}.K0', f'{name}.B0', f'{name}.G1']
    applycal(vis=msname, field=p_calibrator,
             gaintable=gaintables,
             interp=interp_modes(gaintables),
             calwt=False, flagbackup=False)

def apply_calibration(field, use_secondary):
    """ Applies full calibration to given field. """
    gaintables = apriori_tables() + [f'{name}.G3', f'{name}.G1', f'{name}.K0', f'{name}.B0']
    if use_secondary:
        gainfield = secondary_gainfields(gaintables)
    else:
